...
def make_label(
    ...
    required_cols = [text_col] if unit == "conversation" else ["message_id", "message_text"]
    ...
    if cost is not None:
        ...
        logger.warning(f"Unknown pricing for model '{model}' — cost estimate unavailable")
    ...
    if mode == "stream":
        ...
        metadata.update_step(
            step_name="label",
            stats={f"{item_label}_labeled": labeled_count, "classifier": effective_classifier},
        )
